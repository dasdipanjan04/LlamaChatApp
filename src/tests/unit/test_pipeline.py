...
def test_abuse_detection_negative(model_manager: ModelManager):
    ...
    assert is_non_abusive is False, "Non offensive sentences should not be detected as abusive!"
...
async def test_response_generation(model_manager: ModelManager ):
    ...
    assert re.search(r"\bBerlin\b", response, re.IGNORECASE), "Response should mention Berlin."
